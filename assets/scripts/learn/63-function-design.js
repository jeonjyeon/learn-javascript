...
(() => {
  ...
  // 어떤 기능 구현? - 사용자 목록에서 이메일 목록만을 뽑아내고, 새로운 배열을 만들어서 복사
  // 이름만으로 기능을 유추할 수 있게 하려면 어떤 이름이 적절할지? - getUserEmailList
  // 함수는 어떤 매개변수를 전달 받을 지, 어떤 타입일지? 해당 매개변수를 설명한다면? - userList (객체)
  // 함수의 반환값은 어떤 타입이며, 설명한다면? - array / 사용자들의 이메일 주소만을 모아둔 배열?
   ...
   */
  function getUserEmailList(userList) {
    return userList.map(({ email }) => email);
  }

  console.log(getUserEmailList(students));
})();
