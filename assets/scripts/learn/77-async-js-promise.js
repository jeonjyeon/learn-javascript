...
(() => {
  // 판매 가능한 케이크 상자
  const salableCakes = ['스트로베리 초코 생크림', '아이스 망고', '블랙 포레스트', '초코 무스'];
  ...
  // 약속이 이행 또는 거절되기 전 상태
  const promise = buyCake('오리지널 생크림'); // Promise <pending>
  ...
  function buyCake(cakeName) {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      // } else {
      //   reject(`${cakeName} 케익 재고가 없다네요. 아쉽게도 구매할 수 없었습니다. 😢 ⁉️`)
      // }
      ...
      setTimeout(() => {
        existCake ? resolve(`${cakeName} 케익을 구매할 수 있었습니다. 🎂 😊`) : reject(`${cakeName} 케익 재고가 없다네요. 아쉽게도 구매할 수 없었습니다. 😢 ⁉️`);
      }, delayTime);
    });
