...
async function requestPostAndUserAndComments() {
  ...
  // 멀티 리소스 요청
  const [userResponse, commentResponse] = await Promise.all([axios.get(`https://dummyjson.com/users/${userId}?select=firstName,lastName,email`), axios.get(`https://dummyjson.com/comments/post/${postId}`)]);
