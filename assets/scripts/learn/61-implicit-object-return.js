...
() => {
  ...
  // 화살표 함수는 암묵적으로 값을 반환할 때 중괄호(`{}`)를 생략한다.
  // 반면, 객체 또한 중괄호를 표기하므로 화살표 함수를 암묵적으로 사용할 땐 객체 값을 괄호(`()`)로 묶어야 한다.
...
() => {
  ...
  // map() + join() ✅
  const createListItemsTemplate = () => fruits.map((name) => `<li>${name}</li>`).join('');

  console.log(createListItemsTemplate());

  // reduce() ✅
  const createFruitsTemplate = () => fruits.reduce((template, name) => `${template}<li>${name}</li>`, '');
...
(() => {
  ...
  const createFruitsTemplate = () => fruits.map((fruit) => `<li>${fruit}</li>`).join('');
