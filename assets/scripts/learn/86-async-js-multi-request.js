...
(() => {
  ...
  axios.get(`https://dummyjson.com/posts/${randomPostId}`).then(({ data: post }) => {
    const { id, userId } = post;
    console.log({ post, id, userId });

    // 멀티 리소스 요청
    Promise.all([axios.get(`https://dummyjson.com/users/${userId}?select=firstName,lastName,email`), axios.get(`https://dummyjson.com/comments/post/${id}`)])
