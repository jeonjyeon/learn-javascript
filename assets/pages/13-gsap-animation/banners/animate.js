...
(() => {
  ...
  tl.from('.banner', { ease: 'linear', autoAlpha: 0 }).from('.banner__title :first-child', { x: 80, duration: 1 }).from('.banner__title :last-child', { x: -80, duration: 1 }, '<').from('.banner__description', { y: 30 }, '-=0.5').from('.banner__link', { y: 40 }, '-=0.4').from('#items > g', { scale: 0, transformOrigin: 'center', stagger: 0.1 }, '-=0.2');
