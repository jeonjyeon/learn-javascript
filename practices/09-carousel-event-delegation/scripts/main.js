...
(() => {
  ...
  indicatorWrapper.addEventListener('click', (e) => {
    ...
    // console.log('closest', target.closest('.carousel__indicator'))
    ...
    // 2. Element.matches(selector)
    //    엘리먼트(요소)가 전달된 선택자와 일치(매칭)하는 지 확인해 불리언 값을 반환
    // if (target.matches('.carousel__indicator')) {
    //  Element.closest(selector)에서 사용한 코드와 동일
    // }
  });
